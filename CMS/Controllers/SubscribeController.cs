...
namespace CMS.Controllers
{
    ...
    public class SubscribeController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult> GetAll([FromQuery] SubscribeFilter filter)
        {
            try
            {
                return Ok(await _subscribeService.GetAllSubscribe(filter));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetById(int Id)
        {
            try
            {
                return Ok(await _subscribeService.GetById(Id));
            }
            catch (Exception ex)
            {
                Log.Log.Error("Exception: " + ex.ToString(), Log.Log.GetCurrentNameSpace(), Log.Log.GetCurrentClass(), Log.Log.GetCurrentMethod());
